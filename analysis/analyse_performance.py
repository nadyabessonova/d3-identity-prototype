...
INPUT_FILE = "artifacts/performance/raw/performance_results.csv"
METRICS_OUTPUT_FILE = "artifacts/performance/statistics/performance_statistics.csv"
DNS_OUTPUT_FILE = "artifacts/performance/statistics/knot_dns_statistics.csv"
IPFS_OUTPUT_FILE = "artifacts/performance/statistics/ipfs_statistics.csv"
DNSLINK_IPFS_OUTPUT_FILE = "artifacts/performance/statistics/dnslink_ipfs_statistics.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarize D3 prototype performance results."
    )
    parser.add_argument("--input", default=INPUT_FILE)
    parser.add_argument("--metrics-output", default=METRICS_OUTPUT_FILE)
    parser.add_argument("--dns-output", default=DNS_OUTPUT_FILE)
    parser.add_argument("--ipfs-output", default=IPFS_OUTPUT_FILE)
    parser.add_argument(
        "--dnslink-ipfs-output",
        default=DNSLINK_IPFS_OUTPUT_FILE,
    )
    ...
    _write_backend_csv(args.metrics_output, metrics_by_backend)
    _write_csv(args.dns_output, dns_groups)
    _write_csv(args.ipfs_output, ipfs_groups)
    _write_csv(args.dnslink_ipfs_output, dnslink_ipfs_groups)

    print(f"\nWrote {args.metrics_output}")
    if dns_groups:
        print(f"Wrote {args.dns_output}")
    if ipfs_groups:
        print(f"Wrote {args.ipfs_output}")
    if dnslink_ipfs_groups:
        print(f"Wrote {args.dnslink_ipfs_output}")
